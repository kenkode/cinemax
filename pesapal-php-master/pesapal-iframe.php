...
$sel = mysqli_query($con,"select * from movies where id='".$_POST['mid']."'");
...
$reference      = $_POST['ticketno'];                  // unique order id of the transaction, generated by merchant
$first_name     = $_POST['firstname'];                 // optional 
$last_name      = $_POST['lastname'];                  // optional
$email          = $_POST['email'];
$phonenumber    = $_POST['phone'];  

$sel = mysqli_query($con,"select * from movie_periods where movie_id='".$_REQUEST['mid']."'");

$row = mysqli_fetch_array($sel,MYSQLI_ASSOC);    

mysqli_query($con,"insert into booking(firstname,lastname,email,phone,date,ticket_no,movie_id,cinema_id,age,status,id_number)values('".$first_name."', '".$last_name."', '".$email."', '".$phonenumber."','".date('Y-m-d')."', '".$_POST['ticketno']."','".$_REQUEST['mid']."','".$_REQUEST['cinema']."','".$_REQUEST['age']."',0,'".$_REQUEST['idno']."')") or die(mysqli_error($con));	
...
$callback_url = 'http://localhost/onlinecinemabooking/pesapal-php-master/pesapal_callback.php?tno='.$reference.'&id='.$_REQUEST['mid'].'&amount='.$amount;
