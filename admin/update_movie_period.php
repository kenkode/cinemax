...
$id=$_REQUEST['id'];
             ...
             $sel = mysqli_query($con,"select * from movie_periods where id='".$_REQUEST['id']."'");
...
<div class="inner-block">
    <div class="product-block">
      ...
      <div class="col-md-10 col-md-offset-1 compose-right">
			 ...
			 $sql = mysqli_query($con,"update movie_periods set movie_id='".$_REQUEST['movieid']."',cinema_id='".$_REQUEST['cinemaid']."',date='".$date."' where id='".$_REQUEST['id']."'") or die(mysqli_error($con));
			 if( $sql){
			 echo "<img src='images/1433075297_ok-green.png'/><span style='color:green'>Successfully updated movie period!</span>";
			 }else{
			 ...
			 ?>
							...
							<form onSubmit="return validateForm()" name="mp" id="mp" class="com-mail" method="post">
								...
								<select name="movieid" style="width:100%;height:40px">
									...
									<?php
                                      ...
                                      while($rowm=mysqli_fetch_array($selm,MYSQLI_ASSOC)){
									?>
                                    <option value="<?php echo $rowm['id'];?>"<?= ($row['movie_id']==$rowm['id'])?'selected="selected"':''; ?>><?php echo $rowm['name']?></option>
                                    <?php } ?>
								</select>
								...
								<select name="cinemaid" style="width:100%;height:40px">
									...
									<?php
                                      ...
                                      while($rowc=mysqli_fetch_array($selc,MYSQLI_ASSOC)){
									?>
                                    <option value="<?php echo $rowc['id'];?>"<?= ($row['cinema_id']==$rowc['id'])?'selected="selected"':''; ?>><?php echo $rowc['name']?></option>
                                    <?php } ?>
								</select>
	                               ...
	                               <div class="right-inner-addon">
                                    <i class="glyphicon glyphicon-calendar"></i>
	                               	<input type='text' class="form-control" name='date' id='datetimepicker' value="<?php echo $row['date']?>"/>
