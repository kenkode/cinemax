...
<div style="margin-bottom:20px;margin-left:10px;">
      <a href="paymentperiod.php" class="btn btn-success">Payments Report</a>
      </div>
    ...
    $r = mysqli_fetch_array($sel,MYSQLI_ASSOC);
		...
		echo'<tr class="del'.$id.'"><td>'.$i.'</td><td><a href="view_payment.php?id='.$row['id'].'">#'.$row['ticket_no'].'</a></td><td>'.$row['firstname'].' '.$row['lastname'].'</td><td>'.$r['name'].'</td><td>'.number_format($r['price'],2).'</td><td><a class="btn btn-warning" href="view_payment.php?id='.$row['id'].'">View</a></td><td><a target="blank" href="reportpayment.php?id='.$row['id'].'" target="_blank" class="btn btn-success">Report</a></td></tr>';
		}
        ?>
      </tbody>
