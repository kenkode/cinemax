...
$pdf->Write(0, 'Movie Scheduling Report for period between '.$_REQUEST['from'].' AND '.$_REQUEST['to'],'',0,'L',true,0,false,false,0);
...
$tbl_footer = '</table>';
$tbl = '';
$sel=mysqli_query($con,"select movies.name as mname,cinemas.name as cname,movie_periods.id as id,movie_periods.date as date from movie_periods left join movies on movie_periods.movie_id=movies.id left join cinemas on movie_periods.cinema_id=cinemas.id where date between '".$_REQUEST['from']."' And '".$_REQUEST['to']."'") or die(mysqli_error($con));
$i=0;
...
$pdf->Output('Movie Scheduling Report.pdf', 'I');
?>