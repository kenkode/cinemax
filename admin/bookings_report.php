...
$pdf->Write(0, 'Bookings Report for period between '.$_REQUEST['from'].' And '.$_REQUEST['to'],'',0,'L',true,0,false,false,0);
...
$tbl = '';
$total = 0;
$selm=mysqli_query($con,"select * from booking where date between '".$_REQUEST['from']."' AND '".$_REQUEST['to']."'") or die(mysqli_error($con));
                          $i=0;
...
$tbl .= '
   <tr>
   <td colspan="10" align="right"><strong>Total</strong></td><td align="right"><strong>Kshs. '.number_format($total,2).'</strong></td>';

  $tbl .= '</tr>';
// output the HTML content
$pdf->writeHTML($tbl_header . $tbl . $tbl_footer, true, false, false, false, '');

$pdf->Output('bookings_report_'.$_REQUEST['from'].'_'.$_REQUEST['to'].'.pdf', 'I');
?>